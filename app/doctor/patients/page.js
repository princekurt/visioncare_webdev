...
import { FaEdit, FaEye, FaSearch } from 'react-icons/fa'; // Optional: icons make it look premium
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">Manage and view patient</p>
          </div>
          
          {/* Quick Search Placeholder - Looks more professional */}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse">
                {/* Header: Matches Sidebar Charcoal */}
                ...
                <tbody className="divide-y divide-slate-100">
                  {patients.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="px-6 py-4 text-sm font-bold text-[#F17343]">#{p.patient_code}</td>
                      <td className="px-6 py-4 text-sm font-semibold text-[#6D6E70]">{p.patient_name}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{p.patient_gender}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{p.patient_dob}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">
                        <div className="flex flex-col">
                          <span>{p.patient_contact}</span>
                          <span className="text-[10px] text-slate-400">{p.patient_email}</span>
                        </div>
                      </td>
            ...
            <div className="bg-slate-50 px-6 py-4 border-t border-slate-100">
              <p className="text-xs text-slate-500 font-medium">
                Showing <span className="text-[#6D6E70]">{patients.length}</span> registered patients
              </p>
            </div>
          </div>
        )}
